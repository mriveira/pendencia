...
{
	[Authorize]
    [Route("api/pendeciaprioridade/more")]
    public class PendeciaPrioridadeMoreController : Controller
    {

        private readonly IPendeciaPrioridadeRepository _rep;
        private readonly IPendeciaPrioridadeApplicationService _app;
		private readonly ILogger _logger;

        public PendeciaPrioridadeMoreController(IPendeciaPrioridadeRepository rep, IPendeciaPrioridadeApplicationService app, ILoggerFactory logger)
        {
            this._rep = rep;
            this._app = app;
        ...
        public async Task<IActionResult> Get([FromQuery]PendeciaPrioridadeFilter filters)
        {
            ...
            try
            {
				...
				if (filters.FilterBehavior == FilterBehavior.Export)
                {
                    ...
                    var file = export.ExportFile(this.Response, searchResult, "PendeciaPrioridade");
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody]IEnumerable<PendeciaPrioridadeDtoSpecialized> dtos)
        {
