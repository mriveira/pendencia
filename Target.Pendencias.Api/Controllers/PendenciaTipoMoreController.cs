...
{
	[Authorize]
    [Route("api/pendenciatipo/more")]
    public class PendenciaTipoMoreController : Controller
    {

        private readonly IPendenciaTipoRepository _rep;
        private readonly IPendenciaTipoApplicationService _app;
		private readonly ILogger _logger;

        public PendenciaTipoMoreController(IPendenciaTipoRepository rep, IPendenciaTipoApplicationService app, ILoggerFactory logger)
        {
            this._rep = rep;
            this._app = app;
			this._logger = logger.CreateLogger<PendenciaTipoMoreController>();
        }
        ...
        public async Task<IActionResult> Get([FromQuery]PendenciaTipoFilter filters)
        {
            ...
            try
            {
				...
				if (filters.FilterBehavior == FilterBehavior.Export)
                {
                    ...
                    var file = export.ExportFile(this.Response, searchResult, "PendenciaTipo");
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody]IEnumerable<PendenciaTipoDtoSpecialized> dtos)
        {
            ...
            {
                return result.ReturnCustomException(ex,"Target.Pendencias - PendenciaTipo", dtos);
            }

        }

		[HttpPut]
        public async Task<IActionResult> Put([FromBody]IEnumerable<PendenciaTipoDtoSpecialized> dtos)
        {
            var result = new HttpResult<PendenciaTipoDto>(this._logger);
            try
            {
