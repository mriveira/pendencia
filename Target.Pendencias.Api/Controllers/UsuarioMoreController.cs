...
{
	[Authorize]
    [Route("api/usuario/more")]
    public class UsuarioMoreController : Controller
    {
        ...
        public UsuarioMoreController(IUsuarioRepository rep, IUsuarioApplicationService app, ILoggerFactory logger)
        {
            this._rep = rep;
            this._app = app;
			this._logger = logger.CreateLogger<UsuarioMoreController>();
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody]IEnumerable<UsuarioDtoSpecialized> dtos)
        {
            ...
            {
                return result.ReturnCustomException(ex,"Target.Pendencias - Usuario", dtos);
            }

        }

		[HttpPost]
        public async Task<IActionResult> Put([FromBody]IEnumerable<UsuarioDtoSpecialized> dtos)
        {
            var result = new HttpResult<UsuarioDto>(this._logger);
            try
            {
