...
{
	[Authorize]
    [Route("api/comentario/more")]
    public class ComentarioMoreController : Controller
    {
        ...
        public async Task<IActionResult> Get([FromQuery]ComentarioFilter filters)
        {
            ...
            try
            {
				...
				if (filters.FilterBehavior == FilterBehavior.Export)
                {
                    ...
                    var file = export.ExportFile(this.Response, searchResult, "Comentario");
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody]IEnumerable<ComentarioDtoSpecialized> dtos)
        {
            ...
            {
                return result.ReturnCustomException(ex,"Target.Pendencias - Comentario", dtos);
            }

        }

		[HttpPut]
        public async Task<IActionResult> Put([FromBody]IEnumerable<ComentarioDtoSpecialized> dtos)
        {
            var result = new HttpResult<ComentarioDto>(this._logger);
            try
            {
