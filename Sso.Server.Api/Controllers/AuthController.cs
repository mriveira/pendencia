...
namespace Sso.Server.Api.Controllers
{
    ...
    public class AuthController : Controller
    {
        ...
        public string Get()
        {
            ...
            var disco = DiscoveryClient.GetAsync(identityEndPoint).Result;
            return string.Format("identityEndPoint: {0}, TokenEndpoint: {1}", identityEndPoint, disco.TokenEndpoint);
        }
        ...
        public async Task<IActionResult> Post([FromBody]AccountCredencial accountCredencial)
        {
            ...
            if (tokenResponse.IsError)
                return result.ReturnCustomException(new Exception(tokenResponse.ErrorDescription), "Sso.Server.Api - Account");
